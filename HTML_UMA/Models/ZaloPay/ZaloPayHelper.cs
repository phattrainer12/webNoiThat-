...
using System.IO;
using System.Web;
namespace HTML_UMA.Models.ZaloPayHelper
{
    ...
    public class ZaloPayConfig
    {
        ...
        {
            AppId = ConfigurationManager.AppSettings["ZaloPay_AppId"];
            Key1 = ConfigurationManager.AppSettings["ZaloPay_Key1"];
            Key2 = ConfigurationManager.AppSettings["ZaloPay_Key2"];
            CreateOrderUrl = ConfigurationManager.AppSettings["ZaloPay_CreateOrderUrl"];
            QueryOrderUrl = ConfigurationManager.AppSettings["ZaloPay_QueryOrderUrl"];
            GetBanksUrl = ConfigurationManager.AppSettings["ZaloPay_GetBanksUrl"];
            CreateOrderQR = ConfigurationManager.AppSettings["ZaloPay_CreateOrderQR"];
    ...
    {
        public ZaloPayConfig Config { get; set; }
        public ZaloPayHelper(ZaloPayConfig config = null)
        {
            if (config == null)
            {
        ...
        public string CreateOrderURL(
            string appuser, string apptime, string amount, string apptransid,
            string embeddata, string item, string description,
            string phone, string email, string address, string bankcode)
        {
            ...
            param.Add("apptime", apptime);//long|required||thời gian tạo đơn hàng (unix timestamp in milisecond)
            param.Add("amount", amount);//long|required||giá trị của đơn hàng VND
        ...
        public Dictionary<string, object> ParseCallBackResult(string input, out string errorCode)
        {
            ...
            try
            {
                ...
                Dictionary<string, object> i = (Dictionary<string, object>)jsonSerializer.DeserializeObject(input);
                ...
                o = (Dictionary<string, object>)jsonSerializer.DeserializeObject(data);
        ...
        public Dictionary<string, object> QueryOrder(string apptransid)
        {
            string appid = Config.AppId;
            Dictionary<string, string> param = new Dictionary<string, string>();
            ...
            JavaScriptSerializer jsonSerializer = new JavaScriptSerializer();
            Dictionary<string, object> o = (Dictionary<string, object>)jsonSerializer.DeserializeObject(result);
            return o;
        }
        ...
        {
            string json = new JavaScriptSerializer().Serialize(paras);
            return Base64(json);
        }
        ...
        public static string CalculateMac(ZaloPayHMAC algorithm = ZaloPayHMAC.HMACSHA256, string key = "adf", string message = "", bool toHex = true)
        {
